...
const TOOLS: AgentToolDef[] = [
  ...
  },
  {
    ...
    description:
      ...
      ' MANDATORY self-check after EVERY page lands: read the returned <layout-audit>; if it reports text overflow / overlap / out-of-bounds, immediately fix that page with execute_slide_script in the same turn before generating the next page. Never ignore the audit, never declare done while it reports problems.',
    inputSchema: {
      type: 'object',
      properties: {
        topic: {
...
const TEMPLATE_LIBRARY_MODE_NOTE =
  ...
  '- 【跳过 Step 0 Questionnaire】模板库模式下**不要调用 ask_clarification 问卷**（模板已确定版式/配色/风格，问卷无意义且会打断流程）；直接用 search_templates 按用户需求选模板即可。只有搜不到匹配模板、转 generate_deck 自己生成时，才按上游 Step 0 走问卷。\n' +
