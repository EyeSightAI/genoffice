...
interface ChatEntry {
  ...
  error?: string
  streaming?: boolean
  /** the run failed because  is signed out — render an inline sign-in button */
      ...
      .then(async (images) => {
        ...
        if (useTemplateLibrary) {
          modelInstruction +=
            ...
            '\n- 【禁止重新生成】套用模板后，严禁用 generate_deck 重新生成整套 PPT。模板页数太多要精简时，用 apply_ops 的 deleteSlide op 删掉多余页（每页一个 op，至少保留一页）；内容放不下就精简文字或删减页内项目，绝不重做一套。' +
