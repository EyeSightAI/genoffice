/**
 * UToOffice's own  identity: device-code login (office_addin_auth,
 ...
 * subprocesses) to UToOffice. Stored in ~/.genoffice/auth.json, deliberately
...
/** The UToOffice-named api key; '' when not signed in. Cached (invalidated by login/logout). */
...
/**
 * Starts the UToOffice device-code login, cancelling a previous in-flight one
...
/**
 * Signs out of UToOffice only: best-effort server-side revoke of the
